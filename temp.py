...
def generate_intensity_maps(input_zip_path, output_dir):
    ...
    # 1. WRITE VV BAND (GeoTIFF via Write Operator)
    ...
    # 2. WRITE VH BAND (GeoTIFF via Write Operator)
...
# --------------------------------------------------------------------------------------
# --- CORRECTED FUNCTION: Generate Coherence Map (Final Stable GeoTIFF Output) ---
# --------------------------------------------------------------------------------------
def generate_coherence_map(master_zip_path, slave_zip_path, output_dir, polarization="VV"):
    ...
    products_to_close.extend([master, slave])

    # 1. Split
    split_params = HashMap()
    split_params.put("subswath", "IW2")
    split_params.put("selectedPolarisations", polarization)
    master_split = GPF.createProduct("TOPSAR-Split", split_params, master)
    slave_split = GPF.createProduct("TOPSAR-Split", split_params, slave)
    products_to_close.extend([master_split, slave_split])

    # 2. Apply orbit files
    master_orbit = GPF.createProduct("Apply-Orbit-File", HashMap(), master_split)
    slave_orbit = GPF.createProduct("Apply-Orbit-File", HashMap(), slave_split)
    products_to_close.extend([master_orbit, slave_orbit])

    # 3. Coregistration (Back-geocoding)
    bg_params = HashMap()
    bg_params.put("demName", "SRTM 3Sec")
    bg_source_map = HashMap()
    bg_source_map.put("Master", master_orbit)
    bg_source_map.put("Slave", slave_orbit)
    product_bg = GPF.createProduct("Back-Geocoding", bg_params, bg_source_map)
    products_to_close.append(product_bg)
    print("Bands after Back-Geocoding:", list(product_bg.getBandNames()))

    # 4. Interferogram
    product_ifg = GPF.createProduct("Interferogram", HashMap(), product_bg)
    products_to_close.append(product_ifg)
    print("Bands after Interferogram:", list(product_ifg.getBandNames()))

    # Find coherence band dynamically
    band_names = list(product_ifg.getBandNames())
    ...
    product_clean = GPF.createProduct("BandSelect", band_select_params, product_ifg)
    products_to_close.append(product_clean)
    ...
    products_to_close.append(product_final)
    print(" -> Terrain Correction Applied.")

    # --- FINAL GEO TIFF WRITE ---

    # We must use BandSelect again to isolate the coherence band after TC,
    # as auxiliary bands might be present, and write that specific band out.

    # 8. Final Band Select (Isolate the one coherence band)
    write_select_params = HashMap()
    write_select_params.put("sourceBands", jpy.array("java.lang.String", [coherence_band_name]))
    product_write = GPF.createProduct("BandSelect", write_select_params, product_final)
    products_to_close.append(product_write)

    # 9. Save as GeoTIFF (ProductIO.writeProduct on clean product)
    output_path = os.path.join(OUTPUT_DIR, coherence_filename)
    # Write the raw float data directly, relying on JVM memory setting to handle the load.
    ProductIO.writeProduct(product_write, output_path, "GeoTIFF")
...
if __name__ == "__main__":
    ...
    if os.path.exists(POST_EVENT_SLC_ZIP):
        ...
        print(f"FATAL ERROR: Post-event SLC ZIP file not found at {PRE_EVENT_SLC_ZIP}")
